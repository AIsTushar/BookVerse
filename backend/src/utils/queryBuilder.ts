...
class QueryBuilder<T> {
  ...
  // Search
  search(searchableFields: string[], arraySearchFields: string[] = []) {
    const searchTerm = this.query.searchTerm as string;

    if (searchTerm) {
      const normalFields = searchableFields.map((field) => ({
        [field]: { contains: searchTerm, mode: "insensitive" },
      }));

      const arrayFields = arraySearchFields.map((field) => ({
        [field]: {
  ...
  nestedFilterByRange(nestedFilters: rangeFilteringPrams[]) {
    nestedFilters.forEach(
      ({ field, nestedField, maxQueryKey, minQueryKey, dataType }) => {
        ...
        // For nested arrays like "variants.price"
        const [relation, nestedKey] = nestedField.split(".");

        this.prismaQuery.where = {
          ...this.prismaQuery.where,
          [relation]: {
            some: {
              [nestedKey]: rangeCondition,
            },
          },
        };
      },
